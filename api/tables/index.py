...
async def verify_auth_and_permission(
    ...
    has_permission = await permission_manager.check_permission(
        user_info["user_id"], database, schema, operation
    )
...
async def create_table(
    ...
    try:
        ...
        user_info = await verify_auth_and_permission(
            x_api_key, request.database, schema_name, "create"
        )
        ...
        await db_manager.execute_query(
            user_info["user_id"], request.database, create_sql, fetch=False
        )
        ...
        error_response = ErrorResponse(
            error=ErrorDetail(
                code="CREATE_TABLE_ERROR", message=f"Failed to create table: {str(e)}"
            ),
            metadata=MetadataResponse(
                database=request.database,
                schema_name=schema_name,
...
async def get_table_structure(
    ...
    try:
        ...
        rows = await db_manager.execute_query(
            user_info["user_id"], database, query, [schema, table]
        )
        ...
        response = SuccessResponse(
            data={"table": table, "schema": schema, "columns": columns},
            metadata=MetadataResponse(
                database=database,
                schema_name=schema,
                table=table,
...
async def drop_table(
    ...
    try:
        ...
        schema_name = request.schema_name
        user_info = await verify_auth_and_permission(
            x_api_key, request.database, schema_name, "drop"
        )
        ...
        await db_manager.execute_query(
            user_info["user_id"], request.database, drop_sql, fetch=False
        )
