...
from schemas.responses import SuccessResponse, ErrorResponse, MetadataResponse, ErrorDetail
...
def process_query_params(params: Optional[List[Any]]) -> List[Any]:
    ...
    for i, param in enumerate(params):
        ...
        try:
            ...
            elif param_type in ('timestamp', 'datetime', 'datetime.datetime', 'timestamptz'):
                # Convert to datetime
                if isinstance(value, str):
                    if len(value) == 10:  # Just date
                        ...
                        processed_params.append(datetime.fromisoformat(value.replace('Z', '+00:00')))
                    else:
            ...
            raise ValueError(f"Failed to convert parameter {i+1} (value: {value}) to type {param_type}: {str(e)}")
...
async def execute_raw_query(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=408,
                detail=f"Query execution timeout ({timeout} seconds)"
            )
        ...
        error_response = ErrorResponse(
            error=ErrorDetail(
                ...
                details={"query_preview": request.query[:100] + "..." if len(request.query) > 100 else request.query}
            ),
            metadata=MetadataResponse(
                database=request.database,
                timestamp=datetime.utcnow(),
