                ...
                if fetch:
                    ...
                    if isinstance(result, str) and ' ' in result:
                        ...
                        if len(parts) >= 2 and parts[-1].isdigit():
                            return int(parts[-1])
                    return result
            except (asyncpg.QueryCanceledError, asyncpg.IdleInTransactionSessionTimeoutError, asyncpg.IdleSessionTimeoutError) as e:
                await logger.aerror("query_timeout", user_id=user_id, database=database_name, error=str(e))
                raise
            except Exception as e:
                await logger.aerror("query_error", user_id=user_id, database=database_name, error=str(e))
                raise
