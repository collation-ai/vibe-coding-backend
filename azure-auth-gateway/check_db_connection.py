...
async def check_connection():
    ...
    if row:
        ...
        print(f"Encrypted connection exists: {bool(row['connection_string_encrypted'])}")
        ...
        try:
            decrypted = fernet.decrypt(row['connection_string_encrypted'].encode()).decode()
            print(f"Decryption successful")
            print(f"Connection string points to: {decrypted.split('@')[1] if '@' in decrypted else 'Invalid format'}")
