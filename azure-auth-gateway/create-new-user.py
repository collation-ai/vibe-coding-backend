...
async def create_user(username, email, password, organization):
    ...
    try:
        ...
        for perm in permissions_to_grant:
            ...
            print(
                f"  ✓ Granted {perm['permission']} on {perm['database']}.{perm['schema']}"
            )
...
async def grant_schema_permission(username, database_name, schema_name, permission):
    ...
    try:
        ...
        if existing:
            ...
            print(
                f"✓ Updated permission from '{existing['permission']}' to '{permission}'"
            )
        else:
            ...
            print(
                f"✓ Granted {permission} permission on {database_name}.{schema_name} to {username}"
            )
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        asyncio.run(
            add_database_to_existing_user(username, database_name, connection_string)
        )
        ...
        if permission not in ["read_only", "read_write"]:
            ...
            asyncio.run(
                grant_schema_permission(
                    username, database_name, schema_name, permission
                )
            )
